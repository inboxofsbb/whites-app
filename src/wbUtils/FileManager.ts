...
export class FileManager {
  ...
  saveFiles = async ({
    ...
    try {
      const { savedFiles, erroredFiles }: any = await this._saveFiles({
        addedFiles,
      });
      ...
      for (const [fileId] of addedFiles as any) {
  ...
  getFiles = async (
    ...
    try {
      const { loadedFiles, erroredFiles }: any = await this._getFiles(ids);
...
export const encodeFilesForUpload = async ({
  ...
  for (const [id, fileData] of files as any) {
